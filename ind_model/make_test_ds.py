...
class TrafficEnvMod(TrafficEnv):
    ...
    def __init__(self, w=182, h=114, num_agents=..., max_steps=17, pixel2meter=None, make_img=False, img_size=[10,10]):
        ...
        if self.first_step:
            self.bc_models = torch.load(f'bc_agent_ind_{75}_{self.n_modes}_one_step.pth',map_location=device)
        else:
        ...
        #super().__init__(w, h, num_agents, max_steps, pixel2meter, make_img, img_size)
    ...
    def reset(self, num_agents=..., max_steps=17, pixel2meter=None):
        ...
        return super().reset(num_agents, max_steps, pixel2meter)
