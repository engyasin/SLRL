...
if __name__ == "__main__":
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            obs[step] = next_obs
            obs_img[step] = next_img_obs
            dones[step] = next_done

            # ALGO LOGIC: action logic
            with torch.no_grad():
                action, logprob, _, value = agent.get_action_and_value(next_obs,next_img_obs)
            ...
            next_img_obs = torch.Tensor((next_img_obs)).to(device)
        ...
        with torch.no_grad():
            next_value = agent.get_value(next_obs, next_img_obs).reshape(1, -1)
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.batch_size, args.minibatch_size):
                ...
                _, newlogprob, entropy, newvalue = agent.get_action_and_value(b_obs[mb_inds], b_img_obs[mb_inds], action=b_actions[mb_inds].long())#
    ...
    #envs.close()
    writer.close()
    torch.save(agent,f'ppo_agent_unid_image_d_smoothed_{["last_step","frist_step"][envs.first_step]}.pth')
